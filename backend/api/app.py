...
client_cred = SpotifyClientCredentials(client_id=SPOTIPY_CLIENT_ID, \
...
all_features = [
    'id', 'name', 'artists', 'danceability', 'energy', 'loudness', 'mode',
    'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence',
    'tempo'
]

category_dict = {
    'energy': ["energy", "loudness", "tempo", "danceability"],
    'instrumentalness':
    ["instrumentalness", "speechiness", "liveness", "acousticness"],
...
def recommend_song():
    if request.method == 'POST':
        ...
        results = list(
            cosine_similarity(
                np.array(input_vector).reshape((1, -1)),
                sub_song_df).reshape(-1))
        #return results
